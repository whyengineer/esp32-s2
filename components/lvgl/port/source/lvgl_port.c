...
#if 1
    lcdp_write_data_async((uint8_t *)color_p,len);
#else
...
void lvgl_task_init(){
    ...
    lv_color_t *buf1 = (lv_color_t *)heap_caps_calloc(LV_HOR_RES_MAX * 32, sizeof(lv_color_t), MALLOC_CAP_DMA);
    lv_color_t *buf2 = (lv_color_t *)heap_caps_calloc(LV_HOR_RES_MAX * 32, sizeof(lv_color_t), MALLOC_CAP_DMA);
    if((buf1==NULL)||(buf2==NULL)){
        ESP_LOGE(TAG,"buf1:%p,buf2:%p",buf1,buf2);
        ESP_LOGE(TAG,"calloc failed");
        abort();

    }
    ESP_LOGI(TAG,"buf1:%p,buf2:%p",buf1,buf2);
    lv_disp_buf_init(&disp_buf, buf1, buf2, LV_HOR_RES_MAX * 32);   /*Initialize the display buffer*/
